...
public interface AuditCheckListProxy {
	...
	public ResponseEntity<List<QuestionsEntity>> getCheckList(@RequestHeader(name = "Authorization",required = true)String token,
			@RequestBody AuditType auditType );
	
	@PostMapping("/saveResponses")
	public ResponseEntity<List<QuestionsEntity>> saveResponses(@RequestHeader(name = "Authorization",required = true)String token,
